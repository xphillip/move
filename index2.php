        ...
        */
        if($tamanho <= 60000000) {
            if(move_uploaded_file($_FILES['fileUpload']['tmp_name'], $dir.$new_name)) {
                ...
                '<br />'; echo '<font size="+1.5"><div class="label label-warning">'."Link para baixar arquivo ".'</div><br />'.'<input type="textfield" size="60" id="textfield" onclick="this.select()" name="link" value="http://assuncaopromotora.com/move/uploads/'.$new_name.'"></font><br /><p> O link gerado ficará disponivel por 30 dias';};
